...
void GameState::drawGhosts()
{
    if(mAnimClock.getElapsedTime()>sf::seconds(0.2f))
    {
        ...
        for(int i=1; i<=4; ++i)
        {
            ...
            if(mEngine.getScareStatus(i)==pac::PacEngine::Blinking)
            {
...
void GameState::drawAll()
{
    ...
    for(int i=1; i<=4; ++i)
    {
        if(mEngine.getScareStatus(i)!=pac::PacEngine::Dead)
