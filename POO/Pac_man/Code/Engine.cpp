...
{
    if(from.x==1)
        return edy::pac::PacEntity::Right;
    if(from.x==-1)
        return edy::pac::PacEntity::Left;
    if(from.y==1)
        return edy::pac::PacEntity::Down;
    if(from.y==-1)
        return edy::pac::PacEntity::Up;
    return edy::pac::PacEntity::Right;
}
...
void PacEngine::makeWallsMap(sf::RenderTarget& target)
{
    ...
    for(int x=0; x<27; ++x)
    {
        if((mMap[x][0]==Wall)&&(mMap[x+1][0]==Wall))
            drawHorizontalLine(target,x,x+1,0);
        if((mMap[x][30]==Wall)&&(mMap[x+1][30]==Wall))
            drawHorizontalLine(target,x,x+1,30);
    }

    for(int y=0; y<30; ++y)
    {
        if((mMap[0][y]==Wall)&&(mMap[0][y+1]==Wall))
            drawVerticalLine(target,y,y+1,0);
        if((mMap[27][y]==Wall)&&(mMap[27][y+1]==Wall))
            drawVerticalLine(target,y,y+1,27);
    }
    ...
    for(int x=1; x<27; ++x)
        for(int y=1; y<30; ++y)
        {
            ...
            {
                if(mMap[x-1][y]==Wall)
                    drawHorizontalLine(target,x-1,x,y);
                if(mMap[x+1][y]==Wall)
                    drawHorizontalLine(target,x+1,x,y);
                if(mMap[x][y-1]==Wall)
                    drawVerticalLine(target,y-1,y,x);
                if(mMap[x][y+1]==Wall)
                    drawVerticalLine(target,y+1,y,x);
            }
        }
...
int PacEngine::fetchTileAt(sf::Vector2i pos,sf::Vector2i off)
{
    ...
    if(y!=fitBetween(0,y,30)||x!=fitBetween(0,x,27))
        return Wall;
    return mMap[x][y];
}
...
void PacEngine::checkPills()
{
    ...
    if(tmp==0)
        mEventsList.push_back(PacEvent(EventType::PillsExhausted,0));
...
void PacEngine::updateGhost(int who)
{
    ...
    if(guys[who].isAtNode())
    {
        ...
        {
            if(guys[who].getNode()==startPos[who-1]/16)
                guys[who].scared=0;// /16 to get node number from pixel pos
        }
...
sf::Vector2i PacEngine::getTarg(int who)
{
    if(guys[who].scared!=0)
    {
        ...
        {
        case Blinky:
            return guys[Blinky].scared>0?sf::Vector2i(0,0):startPos[0]/16;
        case Inky:
            return guys[Inky].scared>0?sf::Vector2i(28,0):startPos[1]/16;
        case Pinky:
            return guys[Pinky].scared>0?sf::Vector2i(0,31):startPos[2]/16;
        case Clyde:
            return guys[Clyde].scared>0?sf::Vector2i(28,31):startPos[3]/16;
        }
    }
