1.	a) Sã se afiºeze informaþii despre angajaþii al cãror salariu depãºeºte 
...
b) Analog cu cererea precedentã, afiºându-se ºi numele departamentului, media salariilor 
acestuia ºi numãrul de angajaþi. Se cer 2 solu?ii (cu subcerere nesincronizatã în clauza FROM ?i
...
2.	Sã se afiºeze numele ºi salariul angajaþilor al cãror salariu este mai mare 
decât salariile medii din toate departamentele. 
Se cer 2 variante de rezolvare: cu operatorul ALL ?i cu funcþia MAX.
select (avg(salary))
from employees
group by department_id;
...
având cea mai mare vechime din departament. Sã se ordoneze rezultatul dupã numele departamentului.
I - cu sincronizare:
...
8.	Sã se determine locaþiile în care se aflã cel puþin un departament.
Observa?ie: Ca alternativã a lui EXISTS, poate fi utilizat operatorul IN. Scrieþi ºi aceastã variantã de rezolvare.
...
5.	Sã se ob?inã numele salaria?ilor care lucreazã într-un departament în care existã 
...
Observa?ie: Deoarece nu este necesar ca instrucþiunea SELECT interioarã 
(subcererea) sã returneze o anumitã valoare, se poate selecta o constantã (‘x’, 1 etc.).
De altfel, din punct de vedere al performanþei, selectarea unei constante este mai
...
Observa?ie: Se va utiliza NOT EXISTS. Acest exemplu poate fi rezolvat ºi printr-o subcerere necorelatã, 
utilizând operatorul NOT IN (vezi laboratorul 3). Atenþie la valorile NULL! (fie puneþi condiþia
IS NOT NULL în subcerere, fie utilizaþi funcþia NVL). Scrieþi ºi aceastã variantã de rezolvare.
...
7.	Sã se afiºeze codul, numele ºi prenumele angajaþilor care au cel puþin doi subord_directi. 
...
WHERE exists (SELECT COUNT(*)
 ...
 IV. [Clauza WITH]
• Cu ajutorul clauzei WITH se poate defini un bloc de cerere înainte ca acesta sã fie utilizat într-o interogare.
• Clauza permite reutilizarea aceluiaºi bloc de cerere într-o instrucþiune SELECT complexã. 
Acest lucru este util atunci când o cerere face referinþã de mai multe ori la acelaºi bloc de cerere, 
care conþine operaþii join ºi funcþii agregat.

10. Utilizând clauza WITH, sã se scrie o cerere care afiºeazã numele departamentelor 
ºi valoarea totalã a salariilor din cadrul acestora. 
...
11. Sã se afiºeze codul, prenumele ºi numele (pe aceeaºi coloanã), codul job-ului ºi data angajãrii,
ale subordonaþilor direcþi ai lui Steven King care au cea mai mare vechime.
...
11". Sã se afiºeze codul, prenumele ºi numele (pe aceeaºi coloanã), codul job-ului ºi data angajãrii,
ale subalternii subordonaþilor direcþi ai lui Steven King care au cea mai mare vechime.  
...
12. Sã se detemine primii 10 cei mai bine plãtiþi angajaþi.
...
13. Sã se determine cele mai slab plãtite 3 job-uri, din punct de vedere al mediei salariilor acestora.
select *
